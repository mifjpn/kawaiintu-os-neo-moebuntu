...
## 🎥 Real Hardware Tech Demo / 実機デモ動画

Check out how Kawaiintu runs on actual hardware! This quick over-the-shoulder demo showcases the custom Plymouth boot animation, the instant non-destructive theme switching via Python, and the Calamares installer.
(実際のPC上でKawaiintuが動作している様子をご覧ください！カスタムPlymouth（起動画面）から始まり、Pythonフックによる一瞬のテーマ切り替えギミック、そしてインストーラーの起動までを確認できる実機デモです。)

👉 **[Watch the Tech Demo on YouTube](https://www.youtube.com/watch?v=-7wmYkAZRRk)**
